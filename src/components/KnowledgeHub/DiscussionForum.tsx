    ...
    <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 sm:p-8 space-y-6 shadow-2xl animate-scaleUp">
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400 font-mono">Topic: {selectedDiscussion.topicName}</span>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 text-xs text-slate-400 border-b border-slate-800/80 pb-4">
                <img src={selectedDiscussion.authorAvatar || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150"} alt="Avatar" className="w-8 h-8 rounded-full border border-slate-700" />
                <div>
                  <span className="font-bold text-white">{selectedDiscussion.authorName}</span>
              ...
              {/* Question Body */}
              <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 text-xs text-slate-200 leading-relaxed space-y-3 whitespace-pre-line">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedDiscussion.attachments.map(att => (
                      <a
                        key={att.id}
                        href="#"
                        className="px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-blue-400 hover:underline flex items-center gap-2"
                      >
              ...
              <div className="bg-gradient-to-r from-violet-900/20 to-purple-900/20 border border-violet-500/30 p-4 rounded-2xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <Sparkles className="w-5 h-5 text-violet-400 shrink-0" />
                  <div>
                    <p className="text-xs font-bold text-white">AI Learning Copilot Assistance</p>
                    <p className="text-[11px] text-slate-400">Get an instant AI-suggested code solution or discussion summary.</p>
                  </div>
                </div>
                <button
                  onClick={handleAiSuggestAnswer}
                  disabled={aiSuggesting}
                  className="px-3.5 py-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold shrink-0 transition-all shadow-md shadow-violet-600/30"
                >
            ...
            <div className="space-y-4 pt-4 border-t border-slate-800">
              ...
              <div className="space-y-4">
                {selectedDiscussion.answers.map((ans) => (
                  ...
                  >
                    ...
                    {ans.isAccepted && (
                      <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold pb-2 border-b border-emerald-500/20">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        <img src={ans.authorAvatar || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150"} alt="Avatar" className="w-6 h-6 rounded-full" />
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {currentUser.id === selectedDiscussion.authorId && !ans.isAccepted && (
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-emerald-600/20 hover:bg-emerald-600 text-emerald-400 hover:text-white border border-emerald-500/30 text-[10px] font-bold transition-all"
                          >
                    ...
                    <div className="flex items-center gap-4 pt-2 text-xs">
                      <div className="flex items-center bg-slate-900 rounded-lg border border-slate-800 p-1">
                        ...
                        <span className="px-2 font-mono font-bold text-white">{ans.upvotes}</span>
                        <button
                    ...
                    <div className="pl-4 border-l-2 border-slate-800 space-y-2 pt-2">
                      {ans.comments && ans.comments.map(c => (
                        <div key={c.id} className="bg-slate-900/60 p-2.5 rounded-xl text-xs text-slate-300">
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        <input
                          ...
                          className="flex-1 bg-slate-900 border border-slate-800 text-xs rounded-xl px-3 py-1.5 text-white placeholder-slate-500"
                        />
                        <button
              ...
              <form onSubmit={handlePostAnswerSubmit} className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-bold text-white">Your Solution / Answer</h4>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={handleInsertSampleCode}
                      className="px-2.5 py-1 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 text-[11px] text-slate-300 font-mono flex items-center gap-1"
                    >
                ...
                {showCodeInput && (
                  <div className="space-y-1">
                    <span className="text-[10px] font-mono text-slate-400">Code Snippet (C#, SQL, JavaScript...):</span>
                    <textarea
                      rows={3}
                      value={answerCodeSnippet}
                      onChange={(e) => setAnswerCodeSnippet(e.target.value)}
                      placeholder="var result = users.Where(x => x.IsActive);"
                      className="w-full bg-slate-900 border border-slate-800 font-mono text-xs text-emerald-400 rounded-xl p-3 focus:outline-none"
                    />
                  </div>
                )}
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-2xl p-6 space-y-6 shadow-2xl animate-scaleUp">
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              ...
              <button onClick={() => setIsCreateModalOpen(false)} className="p-1 hover:bg-slate-800 rounded-lg text-slate-400">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-300">Technology Topic</label>
                  <select
                    value={newTopicId}
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-300">Description & Context</label>
                <textarea
                  rows={4}
